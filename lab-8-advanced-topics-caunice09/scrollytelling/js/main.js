...
d3.csv('data/assignments.csv').then(_data => {
  ...
  d3.select('#vis-count').text(data.length);
  d3.select('#easy-vis-count').text(data.filter(d => d.difficulty == 'Easy').length);
  d3.select('#difficult-vis-count').text(data.filter(d => d.difficulty == 'Difficult').length);

  const longestAssignment = [...data].sort((a,b) => b.time - a.time)[0];
  d3.select('#max-duration').text(longestAssignment.time);
  d3.select('#max-duration-vis').text(longestAssignment.name);
